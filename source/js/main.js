(() => {
  ...
  function updateSlides() {
    ...
    const slides3 = elementChildren(slidesEl, `.${swiper3.params.slideClass}, swiper-slide`);
    const slidesLength = isVirtual ? swiper3.virtual.slides.length : slides3.length;
    let snapGrid = [];
    const slidesGrid = [];
    ...
    if (gridEnabled) {
      swiper3.grid.initSlides(slides3);
    ...
    for (let i = 0; i < slidesLength; i += 1) {
      slideSize = 0;
      const slide2 = slides3[i];
      if (slide2) {
        if (gridEnabled) {
          swiper3.grid.updateSlide(i, slide2, slides3);
        }
    ...
    if (spaceBetween !== 0) {
      ...
      slides3.filter((_, slideIndex) => {
        if (!params.cssMode || params.loop) return true;
        if (slideIndex === slides3.length - 1) {
          return false;
        }
        return true;
      }).forEach((slideEl) => {
  ...
  function updateSlidesOffset() {
    const swiper3 = this;
    const slides3 = swiper3.slides;
    const minusOffset = swiper3.isElement ? swiper3.isHorizontal() ? swiper3.wrapperEl.offsetLeft : swiper3.wrapperEl.offsetTop : 0;
    for (let i = 0; i < slides3.length; i += 1) {
      slides3[i].swiperSlideOffset = (swiper3.isHorizontal() ? slides3[i].offsetLeft : slides3[i].offsetTop) - minusOffset - swiper3.cssOverflowAdjustment();
    }
  }
  ...
  function updateSlidesProgress(translate2 = this && this.translate || 0) {
    ...
    if (slides3.length === 0) return;
    if (typeof slides3[0].swiperSlideOffset === "undefined") swiper3.updateSlidesOffset();
    ...
    for (let i = 0; i < slides3.length; i += 1) {
      ...
      if (params.cssMode && params.centeredSlides) {
        slideOffset -= slides3[0].swiperSlideOffset;
      }
  ...
  function updateSlidesClasses() {
    ...
    if (isVirtual) {
      ...
      if (gridEnabled) {
        activeSlide = slides3.find((slideEl) => slideEl.column === activeIndex);
        nextSlide = slides3.find((slideEl) => slideEl.column === activeIndex + 1);
        prevSlide = slides3.find((slideEl) => slideEl.column === activeIndex - 1);
      } else {
        activeSlide = slides3[activeIndex];
      }
    }
    if (activeSlide) {
      if (!gridEnabled) {
        ...
        if (params.loop && !prevSlide === 0) {
          prevSlide = slides3[slides3.length - 1];
        }
      }
    }
    slides3.forEach((slideEl) => {
  ...
  function loopCreate(slideRealIndex, initial) {
    ...
    const initSlides = () => {
      const slides3 = elementChildren(slidesEl, `.${params.slideClass}, swiper-slide`);
      slides3.forEach((el, index) => {
        el.setAttribute("data-swiper-slide-index", index);
      });
    };
    const clearBlankSlides = () => {
      const slides3 = elementChildren(slidesEl, `.${params.slideBlankClass}`);
      slides3.forEach((el) => {
        el.remove();
      });
      if (slides3.length > 0) {
  ...
  function loopFix({
    ...
    const gridEnabled = swiper3.grid && params.grid && params.grid.rows > 1;
    if (slides3.length < slidesPerView + loopedSlides || swiper3.params.effect === "cards" && slides3.length < slidesPerView + loopedSlides * 2) {
    ...
    const cols = gridEnabled ? Math.ceil(slides3.length / params.grid.rows) : slides3.length;
    const isInitialOverflow = initial && cols - initialSlide < slidesPerView && !bothDirections;
    let activeIndex = isInitialOverflow ? initialSlide : swiper3.activeIndex;
    if (typeof activeSlideIndex === "undefined") {
      activeSlideIndex = swiper3.getSlideIndex(slides3.find((el) => el.classList.contains(params.slideActiveClass)));
    } else {
    ...
    const activeColIndex = gridEnabled ? slides3[activeSlideIndex].column : activeSlideIndex;
    ...
    if (activeColIndexWithShift < loopedSlides) {
      ...
      for (let i = 0; i < loopedSlides - activeColIndexWithShift; i += 1) {
        ...
        if (gridEnabled) {
          ...
          for (let i2 = slides3.length - 1; i2 >= 0; i2 -= 1) {
            if (slides3[i2].column === colIndexToPrepend) prependSlidesIndexes.push(i2);
          }
        } else {
      ...
      for (let i = 0; i < slidesAppended; i += 1) {
        const index = i - Math.floor(i / cols) * cols;
        if (gridEnabled) {
          slides3.forEach((slide2, slideIndex) => {
    ...
    if (swiper3.params.effect === "cards" && slides3.length < slidesPerView + loopedSlides * 2) {
    ...
    if (isPrev) {
      prependSlidesIndexes.forEach((index) => {
        slides3[index].swiperLoopMoveDOM = true;
        slidesEl.prepend(slides3[index]);
        slides3[index].swiperLoopMoveDOM = false;
      });
    }
    if (isNext) {
      appendSlidesIndexes.forEach((index) => {
        slides3[index].swiperLoopMoveDOM = true;
        slidesEl.append(slides3[index]);
        slides3[index].swiperLoopMoveDOM = false;
      });
    }
    swiper3.recalcSlides();
  ...
  var Swiper = class _Swiper {
    ...
    getSlideIndex(slideEl) {
      ...
      const slides3 = elementChildren(slidesEl, `.${params.slideClass}, swiper-slide`);
      const firstSlideIndex = elementIndex(slides3[0]);
    ...
    slidesPerViewDynamic(view = "current", exact = false) {
      ...
      if (params.centeredSlides) {
        let slideSize = slides3[activeIndex] ? Math.ceil(slides3[activeIndex].swiperSlideSize) : 0;
        let breakLoop;
        for (let i = activeIndex + 1; i < slides3.length; i += 1) {
          if (slides3[i] && !breakLoop) {
            slideSize += Math.ceil(slides3[i].swiperSlideSize);
            spv += 1;
        ...
        for (let i = activeIndex - 1; i >= 0; i -= 1) {
          if (slides3[i] && !breakLoop) {
            slideSize += slides3[i].swiperSlideSize;
            spv += 1;
            if (slideSize > swiperSize) breakLoop = true;
          }
        }
    ...
    update() {
      ...
      if (params.freeMode && params.freeMode.enabled && !params.cssMode) {
        ...
        if ((params.slidesPerView === "auto" || params.slidesPerView > 1) && swiper3.isEnd && !params.centeredSlides) {
          const slides3 = swiper3.virtual && params.virtual.enabled ? swiper3.virtual.slides : swiper3.slides;
          translated = swiper3.slideTo(slides3.length - 1, 0, false, true);
        } else {
  ...
  var buttonPrev = section.querySelector(".slider-navigation__button--prev");
  var buttonNext = section.querySelector(".slider-navigation__button--next");
  buttonPrev.addEventListener("click", () => swiper.slidePrev());
  buttonNext.addEventListener("click", () => swiper.slideNext());

  // source/js/modules/directions-slide.js
  ...
  var mediaQuery2 = window.matchMedia("(max-width: 1023px)");
  var handleMediaChange2 = (element) => {
    if (element.matches) {
      if (!swiper2) {
        createSwiper2();
      }
  ...
  var section2 = document.querySelector(".promo-banner");
  var buttonPrev2 = section2.querySelector(".slider-navigation__button--prev");
  var buttonNext2 = section2.querySelector(".slider-navigation__button--next");
  ...
  var controlsBanner = document.querySelector(".promo-banner__controls");
  var controls = controlsBanner.querySelectorAll(".banner-control");
  var slides2 = document.querySelectorAll(".banner-slide");
  var parentSlides = [];
